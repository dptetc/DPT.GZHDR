...
void WINAPI glTexParameteri( GLenum target, GLenum pname, GLint param )
{
 ...
 // Need for Zandronum SkyFix (helps with SSAO)
bool ZandronumSkyFixStarted=0;
unsigned ZandronumSkyFixSkipCounter=0;

void WINAPI glDisable( GLenum cap )
{
	// Zandronum SkyFix
	// Disable all invisible walls drawing in ZBuffer - like sky (need for SSAO)
	if (QFXRenderer::Instance().GetZandronumSkyFix()) {
		
		// Disable textures? I know what you want! xD
		if ( cap == GL_TEXTURE_2D && !ZandronumSkyFixStarted && QFXRenderer::Instance().GetRenderFrame()==2)
			if (ZandronumSkyFixSkipCounter++ > 0) {
				ZandronumSkyFixStarted=1; 

				// Just push out this crap!
				gl::qglEnable( GL_DEPTH_TEST );
				gl::qglDepthFunc( GL_NEVER ); 
			}

		// And don't let any changes
		if (ZandronumSkyFixStarted) return;
	}
...
void WINAPI glEnable( GLenum cap )
{
	...
	if ( cap == GL_TEXTURE_2D && ZandronumSkyFixStarted) {
		
		// I excuse you, but not do this anymore :)
		ZandronumSkyFixStarted=0;
...
{
	ZandronumSkyFixSkipCounter=0; // Need for Zandronum SkyFix (helps with SSAO)
	gl::CheckInit();
