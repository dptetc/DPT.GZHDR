...
QFXRenderer :: QFXRenderer()
{
	...
	m_varRenderFrame = QFXSettings::Instance().GetInteger( "RenderFrame", 0 );
	...
	m_varBloomNumSteps = min( 16, QFXSettings::Instance().GetInteger( "BloomSteps", 8 ) );
	...
	m_varSSAODistance = QFXSettings::Instance().GetFloat( "SSAODistance", 1.0f );
	m_varSSAOMinZ = QFXSettings::Instance().GetFloat( "SSAOMinZ", 0.005f );
	m_varSSAOMaxZ = QFXSettings::Instance().GetFloat( "SSAOMaxZ", 1.0f );
	m_varZandronumSkyFix = QFXSettings::Instance().GetInteger( "ZandronumSkyFix", 0 );
	...
	m_varDOFMaxZ = QFXSettings::Instance().GetFloat( "DOFMaxZ", 1.0f );
}
...
void QFXRenderer :: PostProcess_SSAO( void )
{
	...
	m_pSSAORenderShader->SetParameter4f( 1, m_varSSAOMinZ, m_varSSAOMaxZ, m_varSSAOScale, m_varSSAODistance );
...
void QFXRenderer :: PostProcess_DOF( void )
{
	...
	m_pDOFDepthShader->SetParameter4f( 0, 2.0f * m_zNear, m_zFar - m_zNear, m_varDOFMaxZ, 0 );
	RenderScaledFSQ( blurTexWidth, blurTexHeight, screenWidth, screenHeight );
	qglBindTexture( GL_TEXTURE_RECTANGLE_ARB, m_uiBlurDepth );
	qglCopyTexSubImage2D( GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, 0, m_WindowSize[1] - blurTexHeight, blurTexWidth, blurTexHeight );
	...
	m_pDOFRenderShader->SetParameter4f( 0, m_varDOFMinZ, m_flLastDepth, m_varDOFScale, m_varDOFMaxZ );
	qglBindTexture( GL_TEXTURE_RECTANGLE_ARB, m_uiScreenRGB );
	qglActiveTextureARB( GL_TEXTURE1_ARB );
	qglBindTexture( GL_TEXTURE_RECTANGLE_ARB, m_uiBlurTexture );
	qglActiveTextureARB( GL_TEXTURE2_ARB );
